    ...
    if (this.options.drawXAxis) {
        if (this.layout.xticks) {
            var drawTick = function(tick) {
				...
				// Magic Customization For Highlighting The Turn When Assessment Was Done
				if (typeof(gaintHighlightedItems) != 'undefined' && gaintHighlightedItems != null)
				{
					for (var intCtr=1; intCtr < gaintHighlightedItems.length; intCtr++)
					{
						if(gaintHighlightedItems[intCtr] == "")
						{
						}
						else if (gaintHighlightedItems[intCtr] == tick[1])
						{
							label.style.fontWeight = "bold";
							label.style.textDecoration = "underline";
							label.style.color = "blue";
							label.style.backgroundColor = "#d6d629";
						}
					}
				}				
				// Magic Customization Over
